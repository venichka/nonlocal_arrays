...
# ╔═╡ 267de2ad-89cd-4860-a2b6-c4b12aa45b7a
begin
...
# ╔═╡ 10bf3d04-ffa3-11ef-3573-9706a18cd705
md"""
# One atom with Zeeman split derivation
"""

# ╔═╡ b929dc8d-cf02-4a67-a556-1cbd653f3841
begin
	# Define parameters
	# @cnumbers w_m1 w_0 w_p1 g gc γ 
	γ, g_m1, gc_m1, g_0, gc_0, g_p1, gc_p1, w_m1, w_0, w_p1 = cnumbers("γ g_{-1} g_{-1}^{*} g_{0} g_{0}^{*} g_{+1} g_{+1}^{*} ω_{-1} ω_{0} ω_{+1}")
	
	# Define hilbert space
	ha = NLevelSpace(:atom, (:g, Symbol("\\sigma_-"), :π, Symbol("\\sigma_+")))
	h = ha
	
	# Define the fundamental operators
	s_m1 = Transition(h,:J,:g,Symbol("\\sigma_-"))
	s_0 = Transition(h,:J,:g,:π)
	s_p1 = Transition(h,:J,:g,Symbol("\\sigma_+"))
	
	# Hamiltonian
	H_a = (w_m1*s_m1'*s_m1 + 
		   w_0*s_0'*s_0 + 
		   w_p1*s_p1'*s_p1)
	H_f = -(g_m1*s_m1 + gc_m1*s_m1' + 
			g_0*s_0 + gc_0*s_0' +
			g_p1*s_p1 + gc_p1*s_p1')
	H = H_a + H_f
...
# ╔═╡ 611ada64-e624-4b10-bd98-aaeab1998066
begin
	# list of operators
	n_order = 1
	ops = (n_order > 1) ? [s_m1, s_m1'*s_m1] : [s_m1]
...
# ╔═╡ 2b31573d-28fc-45fc-8966-c3e3a9c665da
eqs_comp = complete(eqs; simplify=true) #automatically complete the system

# ╔═╡ ff019532-dfc7-44eb-99d8-b74ccdefb999
begin
	# Numerical solution
	ps = (γ, g_m1, gc_m1, g_0, gc_0, g_p1, gc_p1, w_m1, w_0, w_p1)
	@named sys = ODESystem(eqs_comp)
	u0 = zeros(ComplexF64, length(eqs_comp))
	p0 = (0.0833, -0.4, -0.4, 
		  0.565685424949238im, -0.565685424949238im, 0.4, 0.4,
		  -0.5, 0.0, 0.5)
	prob = ODEProblem(sys,u0,(0.0,200.0),ps.=>p0)
	sol = solve(prob)
end

# ╔═╡ 5e47f731-1225-416f-935d-7458631114b2
begin
	# Interpolate solution
	τ = range(0.0, sol.t[end], length=15001)
	sol_int = sol.(τ)
	inds = [2,4,5]
end

# ╔═╡ 287be7fd-a29c-4b56-9b87-df4a8eb0e496
begin
	plot(τ, [real(sol_int[i][inds[1]]) for i in eachindex(τ)])
	plot!(τ, [real(sol_int[i][inds[2]]) for i in eachindex(τ)])
	plot!(τ, [real(sol_int[i][inds[3]]) for i in eachindex(τ)])
end
...
manifest_format = "2.0"
project_hash = "0951f54aaa44213f94ce57395a4420a94e591ee3"
...
git-tree-sha1 = "3747f7e2f54774a175bba06098629062bcb01950"
...
git-tree-sha1 = "eef118dce9dd0984aee2c45d34a91096b2cb9455"
uuid = "35bcea6d-e19f-57db-af74-8011de6c7255"
version = "0.3.5"

[[deps.QuantumInterface]]
deps = ["LinearAlgebra", "SparseArrays"]
git-tree-sha1 = "c1ce761a5c2cf6168aa148cc49e5d08b6fbf4295"
uuid = "5717a53b-5d69-4fa3-b976-0bf2f97ca1e5"
version = "0.3.7"

[[deps.QuantumOpticsBase]]
deps = ["Adapt", "FFTW", "FastExpm", "FastGaussQuadrature", "FillArrays", "LRUCache", "LinearAlgebra", "QuantumInterface", "Random", "RandomMatrices", "RecursiveArrayTools", "SparseArrays", "Strided", "UnsafeArrays"]
git-tree-sha1 = "288832cb7cc2b2987795eddee5b888204f1e89a6"
...
git-tree-sha1 = "bda149bef2e52fe889b825777ebd1724aea4566c"
